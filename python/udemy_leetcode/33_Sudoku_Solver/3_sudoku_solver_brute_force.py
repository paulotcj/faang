...
class Solution:
    ...
    def is_valid(self, row : int, col : int , c : str) -> bool:
        ...
        starting_block_row = (row//3) * 3 # e.g.: 4 // 3 = 1 -> 1 * 3 = 3
        starting_block_col = (col//3) * 3 # e.g.: 5 // 3 = 1 -> 1 * 3 = 3
        
        '''We got 'c', now we have to check for all the 9 positions in the current row, all the
        9 positions in the current column, and all the 9 positions in the current block'''
        #-----------------------------------
        for i in range(self.board_width): # loop 9 times from 0 to 8
            ...
            probing_row_i = starting_block_row + ( i // 3) # the row only needs to change every 3rd 'i'th
            probing_col_i = starting_block_col + (i % 3 )  # the column needs to change every 'i'th but resets every 3rd 'i'th
            
            if self.board[probing_row_i][probing_col_i] == c : 
                return False
