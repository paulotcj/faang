    ...
    def numIslands(self, grid: List[List[str]]) -> int:
        ...
        for r_idx, row in enumerate(grid):
            for c_idx, col in enumerate(row):
                ...
                #let's look around this cell to see if there are any other cells that are part of this island
                self.enqueue_directions(r_idx, c_idx, grid)
                self.process_queue(grid, currently_island = self.seen[r_idx][c_idx])
        #----------------
        return self.num_islands
    #-------------------------------------------------------------------------
    #-------------------------------------------------------------------------
    def process_queue(self, grid: List[List[int]], currently_island: bool) -> None:
        while self.queue:
            row, col = self.queue.pop(0)
            if grid[row][col] == '1' and currently_island:
